...
const all = async () => {
  ...
  await copyCSS(path.join(__dirname, '../../node_modules/highlight.js/styles', config.highlight.style + '.css'), path.join(destination, config.build.paths.assets, 'css', 'highlight.css'))
  ...
  const res = perf.stop()
  console.log('We needed', chalk.yellow(res.words) + '.')
}
