    ...
    try {
      if (options.minify) {
        if (path.basename(src).indexOf('.min.js') === -1) {
          ...
          console.log(path.basename(src), 'has been copied and minified.')
        } else {
          await fs.copy(src, path.join(dest, path.basename(src)))
          console.log(path.basename(source), 'has been copied.')
        }
      } else {
        await fs.copy(src, path.join(dest, path.basename(src)))
        console.log(path.basename(src), 'has been copied.')
      }
    } catch (e) {
...
const renderSASS = async (options) => {
  ...
  if (fs.existsSync(src)) {
    let files = await getFiles(path.join(src, '**/*.s[ac]ss'))
    files.forEach(f => {
        ...
        sass.render({ file: f }, (err, res) => {
          if (!err) {
            ...
            fs.writeFile(path.join(dest, path.basename(f).substr(0, path.basename(f).indexOf('.')) + ((options.minify) ? '.min.css' : '.css')), css, (e) => {
              if (!e) {
                console.log(path.basename(f), 'has been rendered.')
              } else {
                console.error(e)
              }
            })
...
const all = async () => {
  ...
  // Copy JS SEPERATE
  const files = await getFiles(path.join(source, config.build.paths.assets, 'js', '**/*.js'))
  ...
  await render(path.join(source, config.build.paths.content, 'index.md'))
}
